...
{
    [Description("销售出库单反审核时，删除多方交易单据")]
    [HotUpdate]
    public class OutStockUnAuditOperation : AbstractOperationServicePlugIn
    {
        ...
        public override void EndOperationTransaction(EndOperationTransactionArgs e)
        {
            foreach (DynamicObject item in e.DataEntitys)
            {
                if (item != null)
                {
                    DynamicObjectCollection muiPartBuinessColl = item["F_MLD_MuiPartBill"] as DynamicObjectCollection;
                    ...
                    {
                        if (coll["F_MLD_BuinessType"].ToString().EqualsIgnoreCase("销售出库单"))
                        {
                    ...
                    //反审销售出库，
                    UnAudit("SAL_OUTSTOCK",outBillIdList, outBillNoList);
                    Delete("SAL_OUTSTOCK", outBillIdList, outBillNoList);

                    //反审核采购入库的下游单据 应付单
                    AfterBillOperation("T_AP_PAYABLEENTRY", "AP_Payable", inStockBillNoList);
                    //再反审采购入库
                    UnAudit("STK_InStock", inStockBillIdList, inStockBillNoList);
                    Delete("STK_InStock",inStockBillIdList, inStockBillNoList);
        ...
        {
            String inserSql = String.Format(@"delete from T_MLD_MuiPartBill where FID={0}",FID);
