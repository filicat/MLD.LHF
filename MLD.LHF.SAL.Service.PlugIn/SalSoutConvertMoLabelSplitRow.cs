...
namespace MLD.LHF.SAL.Service.PlugIn
{
    ...
    public class SalSoutConvertMoLabelSplitRow : AbstractConvertPlugIn
    {
        ...
        {
            public long MoRptEntryId { get; set; }
        ...
        public override void AfterConvert(AfterConvertEventArgs e)
        {
            ...
            JSONArray labelJA;
            if (!this.Option.TryGetVariableValue<JSONArray>("mld_labels", out labelJA)) return;
            // 反序列化为 List<Entry>
            ...
            Dictionary<long, List<LabelReq>> labelReqsMap = labelReqs
                .Where(r => r.MoRptEntryId > 0 && r.Label.Length > 0 && r.PackageQty > 0)
                .GroupBy(req => req.MoRptEntryId) // 按 EntryId 分组
            ...
            foreach (var billDynObjEx in billDynObjExs) //循环数据包扩展集合
            {
                ...
                for (int i2 = 0; i2 < entryDynObjs.Count; i2++)
                {
                    ...
                    foreach (var item in dicSplitInfo)
                    {
                        ...
                        tView.Model.SetValue(lotField, dynObj, listLabelReq[0].Label);
                        tView.InvokeFieldUpdateService(lotField.Key, rowIndex);
                        tView.Model.SetValue(field, dynObj, listLabelReq[0].PackageQty);
                        ...
                        for (int i = 0; i < item.Value.Item1; i++)
                        {
                            ...
                            decimal qty = listLabelReq[i + 1].PackageQty;
                            tView.Model.SetValue(field, entryDynObjs[newRowIndex], qty);
                            ...
                            foreach (var fieldProperty in this._lstProperys)
                            {
                                ...
                                {
                                    fieldProperty.SetValue(linkObj, qty);
